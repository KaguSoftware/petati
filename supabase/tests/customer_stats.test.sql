...
from public.customers c where c.email = 'customer@petati.local';
insert into public.orders (store_id, number, customer_id, email, currency, status, subtotal, total)
...
select is((select orders_count from public.v_customer_stats where email = 'customer@petati.local'), 1,
  'cancelled orders are not counted');
select is((select total_spent from public.v_customer_stats where email = 'customer@petati.local'), 1500::bigint,
  'total_spent sums paid orders only');
select ok((select last_order_at is not null from public.v_customer_stats where email = 'customer@petati.local'),
