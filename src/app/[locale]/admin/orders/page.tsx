...
async function OrdersList({ locale, searchParams }: { locale: string; searchParams: Props["searchParams"] }) {
  ...
  if (isPlainList(sp, ["status"])) {
    ...
    const panels = BUCKETS.map((b, i) => {
      ...
      return {
        ...
        content: (
          <>
            <OrdersTable rows={pages[i].rows} locale={ctx.locale} sort={{ sort: list.sort, dir: list.dir }} query={query} storeId={canUpdate ? ctx.store.id : undefined} scope={canUpdate ? `orders:${b}` : undefined} />
  ...
  return (
    <>
      ...
      <OrdersTable rows={rows} locale={ctx.locale} sort={{ sort: list.sort, dir: list.dir }} query={query} storeId={canUpdate ? ctx.store.id : undefined} scope={canUpdate ? `orders:${status ?? "all"}` : undefined} />
