...
import { getCustomer } from "@/lib/admin/customers/queries";
...
async function Content({ params, searchParams }: { params: Props["params"]; searchParams: Props["searchParams"] }) {
  ...
  const { rows, total } = await listOrders(ctx.store.id, { ...list, customerId: customer.id });
  ...
  return (
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <KpiCard label={t("customers.orders")} value={num.format(customer.orders_count)} icon={ShoppingBag} />
