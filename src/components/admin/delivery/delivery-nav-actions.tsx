import { Coins, Route, ScrollText, Settings2, Users } from "lucide-react";
import { getTranslations } from "next-intl/server";
import { buttonVariants } from "@/components/ui/button";
import { Link } from "@/i18n/navigation";

/** Base UI buttons have no `asChild`, so a link that looks like a button uses buttonVariants(). */
export async function DeliveryNavActions({ canManage }: { canManage: boolean }) {
  const t = await getTranslations("admin.delivery");
  return (
    <>
      <Link href="/admin/delivery/runs" className={buttonVariants({ variant: "outline", size: "sm" })}>
      ...
      {canManage && (
        <Link href="/admin/settings?tab=delivery" className={buttonVariants({ variant: "ghost", size: "icon-sm" })} aria-label={t("settingsLink")} title={t("settingsLink")}>
          <Settings2 />
        </Link>
      )}
    </>
