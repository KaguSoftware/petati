        ...
        <ul className="divide-y divide-foreground/15">
          {reviews.map((r) => (
            <li key={r.id} className="flex flex-col gap-3 py-8 first:pt-0">
              {r.title && <p className="bidi-auto font-serif text-2xl font-medium leading-snug text-balance @tablet:text-3xl">{r.title}</p>}
              {r.body && <p className={"bidi-auto " + (r.title ? "max-w-prose text-sm leading-relaxed text-muted-foreground" : "max-w-prose font-serif text-xl leading-snug")}>{r.body}</p>}
