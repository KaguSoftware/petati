...
export function NewsletterEditorial({ title, subtitle, formSlot }: NewsletterProps) {
  return (
    <section className="border-y border-foreground/15 bg-background">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-14 @tablet:grid-cols-2 @tablet:gap-0 @tablet:py-20">
        <div className="flex flex-col gap-3 @tablet:border-e @tablet:border-foreground/15 @tablet:pe-12">
          ...
          <h2 className="font-serif text-3xl font-medium tracking-tight text-balance @tablet:text-5xl">{title}</h2>
        </div>
        <div className="flex flex-col justify-center gap-5 @tablet:ps-12">
          <p className="max-w-md font-serif text-lg italic text-muted-foreground">{subtitle}</p>
