...
export function HeroMinimal({ title, subtitle, ctaLabel, ctaHref, imageUrl }: HeroProps) {
  return (
    <section className="relative isolate overflow-hidden bg-foreground text-white">
      <div className="relative aspect-[4/5] max-h-[44rem] w-full @tablet:aspect-[16/7]">
        {imageUrl ? (
        ...
        <div className="absolute inset-x-0 bottom-0">
          <div className="mx-auto flex max-w-7xl flex-col items-start gap-4 px-4 pb-10 @tablet:gap-5 @tablet:pb-16">
            <h1 className="max-w-2xl text-4xl font-semibold tracking-tight text-balance @tablet:text-6xl">{title}</h1>
            <p className="max-w-xl text-base text-white/85 @tablet:text-lg">{subtitle}</p>
            <Link href={ctaHref} className={cn(buttonVariants({ size: "lg" }), "bg-white text-foreground hover:bg-white/90")}>
              {ctaLabel}
            </Link>
          </div>
        </div>
