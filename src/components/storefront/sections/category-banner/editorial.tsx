...
export function CategoryBannerEditorial({ title, categories }: CategoryBannerProps) {
  ...
  return (
    <section className="mx-auto max-w-7xl border-t border-foreground/15 px-4 py-12 @tablet:py-16">
      ...
      <ul className="flex flex-col divide-y divide-foreground/15 border-y border-foreground/15 @tablet:flex-row @tablet:flex-wrap @tablet:divide-x @tablet:divide-y-0">
        {categories.map((c) => (
          <li key={c.id} className="@tablet:flex-1">
            <Link
              href={`/c/${c.slug}`}
              className="block py-4 font-serif text-2xl font-medium tracking-tight transition-colors hover:text-primary focus-visible:text-primary focus-visible:outline-none @tablet:px-6 @tablet:py-6 @tablet:text-center @tablet:text-3xl @desktop:text-4xl"
            >
              {c.name}
            </Link>
          </li>
